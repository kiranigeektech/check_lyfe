...
class clubCategory {
  getCategory = async (request) => {
    try {
      const category = await db.clubCategory.findAll({where:{isDeleted:false}});
      return category;
    } catch (e) {
      console.log("ss", e);
    }
  ...
  addCategory = async (request) => {
    try {
      ...
      for (let i = 0; i < request.payload.categoryName.length; i++) {
        let checkAlreadyExist = await db.clubCategory.findOne({
          where: {
      ...
      const add = await db.clubCategory.bulkCreate(uniqueCategory);
  ...
  deleteCategory = async (request) => {
    try {
      const delCategory = await db.clubCategory.destroy({
        where: {
...
module.exports = new clubCategory();
